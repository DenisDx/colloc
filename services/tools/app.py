...
def _truncate_for_log(value: Any, limit: int = 500) -> Any:
    """Trim large values for log payloads. Output: compact value. Input: any value."""
    if isinstance(value, str):
        return value if len(value) <= limit else f"{value[:limit]}..."
    if isinstance(value, list):
        return [_truncate_for_log(item, limit) for item in value[:10]]
    if isinstance(value, dict):
        return {key: _truncate_for_log(item, limit) for key, item in list(value.items())[:20]}
    return value
...
def _is_enabled(env_keys: list[str]) -> bool:
    ...
    if os.getenv("TOOLS_ENABLED", "false").strip().lower() not in ("true", "1", "yes"):
        return False
    for key in env_keys:
        if os.getenv(key, "").strip().lower() in ("true", "1", "yes"):
            return True
    return False
...
async def invoke_tool(request: ToolInvokeRequest) -> dict[str, Any]:
    ...
    try:
        ...
        status = "http_error"
        error_text = exc.detail if isinstance(exc.detail, str) else str(exc.detail)
        raise
        ...
        append_system_log(
            ...
            "Tool invocation finished.",
            {
                ...
                "error": _truncate_for_log(error_text),
            },
        )
